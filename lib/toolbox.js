...
//                   变成数字的唯一手段。**（注意：它目前只测差分那条链，没有测
//                   see_screen 那条——那正是最需要数字的地方。）
//   vision_storage  附件库的规模报告与可选清理
//
// 【为什么这里没有"看图"工具】
//   原来有一个 see_image：读一个图片文件，让视觉模型描述它。它就是内置
//   `read_image` 的重复实现，而且是较差的那个（多一层模型间转述）。既然模型现在
//   自带视觉能力，这件事交给内置工具，不再由本插件提供。
//
// 【差分为什么不让模型去看两张原图】
