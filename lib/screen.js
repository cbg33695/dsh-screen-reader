...
export function install(ctx) {
  ...
  const state = {
    ...
    // 会话模型能否收图。按 provider/model 缓存——用户中途换模型时必须重新判断，
    // 缓存一份不带上限的结论会让插件在新模型上继续走错路。
    imageSupport: { key: '', value: null },
  }
  ...
  async function capture(windowQuery, region) {
    ...
    const empty = { ok: false, error: '', path: '', sig: '', width: 0, height: 0, source: '', sourceSize: '', region: '', nonBlack: '', mode }
    if (!run.ok) {
    ...
    const result = {
      ...
      // 三个字段刻意分开，别再合并：
      //   source     抓的是什么（人读的描述，不含任何尺寸）
      //   sourceSize 抓取面的尺寸（1920x1080 这种，属于"源"）
      //   size       写出来的 PNG 尺寸（裁切后，才是调用方真正拿到的东西）
      // 它们曾经挤在一个叫 TARGET 的字段里，于是源尺寸会被读成图幅——
      // 真发生过：报 1920x1080，实际文件 1190x486。
      // SOURCE= 是新字段；|| TARGET= 是给"脚本已升级、模块还没重载"那个窗口兜底的
      // （ps1 每次调用现执行，模块只在启动时加载）。TARGET 现在只带描述、不带尺寸，
      // 所以这条兜底不会把那个误导性的数字带回来。
      source: pick(run.text, 'SOURCE=') || pick(run.text, 'TARGET='),
      sourceSize: pick(run.text, 'SOURCESIZE='),
      region: pick(run.text, 'REGION='),
      nonBlack,
      mode,
    }
  ...
  // 当前会话用的是哪个模型，它收不收图。
  //
  // 会话模型在 `agent.options.model` 上（dsh-agent-loop 用它填 {{model}} 提示词变量），
  // 执行期通过 agents.currentInitiator() 拿得到——已实测：返回 Agent，options 里是
  // { provider, model }。拿到之后再问 llm 这条路由的 inputModalities。
  //
  // 三种结果都要能区分：accepts=true（能收图，走图像路径）、
  // known=true 且 accepts=false（确认收不了，必须转录）、
  // known=false（问不出来）。**问不出来时按"收不了"处理**：给收不了的模型发图会让
  // 整个请求抛错，代价是整轮对话失败；而多走一次转录最坏只是少一张原图。风险不对称。
  async function sessionImageSupport() {
    let provider = ''
    let model = ''
    try {
      ...
      const agent = agents !== undefined && agents !== null && typeof agents.currentInitiator === 'function'
        ...
        : undefined
      const opts = agent !== undefined && agent !== null ? agent.options : undefined
      if (opts !== undefined && opts !== null && typeof opts.model === 'string') {
        model = opts.model
    ...
    if (state.imageSupport.value !== null && state.imageSupport.key === key) return state.imageSupport.value

    const value = { accepts: false, known: false, provider, model }
    if (model !== '') {
      const llm = ctx.get('llm')
      if (llm !== undefined && llm !== null) {
        let candidates = provider !== '' ? [provider] : []
        if (candidates.length === 0) {
          try {
            candidates = (await llm.listProviders())
              .map((p) => (p !== undefined && p !== null ? String(p.id || '') : ''))
              .filter((id) => id !== '')
          } catch {
            candidates = []
          }
        ...
        for (const pid of candidates) {
          ...
          if (!models.some((m) => m !== undefined && m !== null && String(m.id || '') === model)) continue
          try {
            const info = await llm.resolveModelInfo(pid, model)
            const raw = info !== undefined && info !== null
              ? (info.inputModalities || (info.model !== undefined && info.model !== null ? info.model.inputModalities : null))
              : null
            if (raw !== null && raw !== undefined) {
              value.known = true
              value.accepts = Array.isArray(raw) && raw.includes('image')
              value.provider = pid
            }
          } catch {
            // 解析失败 → known 保持 false → 按"收不了"处理
          }
          break
        }
      }
    ...
    state.imageSupport.key = key
    state.imageSupport.value = value
    return value
  }
  ...
  async function observe(focus, windowQuery, region, wantText) {
    const result = {
      ...
      truncated: false, source: '', sourceSize: '', region: '', size: '', nonBlack: '', blank: false,
      locationKind: '', suggestedRegion: '', locationNote: '',
      imageRef: null, bytes: 0,
      visionPath: '', sessionModel: '', note: '',
    }
    ...
    // ── 走哪条路，由"当前会话的模型收不收图"决定，而不是由调用方猜 ──────────────
    //
    // 为什么必须在这里判断：provider 发送前会扫 messages 里有没有 image，只要模型没有
    // 声明 inputModalities 含 image 就**直接抛 LlmError**（dsh-llm-deepseek L1602-1605），
    // 整个请求失败——模型连正文都看不到。实测本机 6 个模型里只有
    // deepseek-v4-flash-vision-exp 收图，deepseek-v4-flash / v4-pro / 本地 qwen3 三个
    // 全都会踩这一条。以前这里默认返回图、把"能不能收"留给调用方判断，结果是纯文本
    // 会话一调就炸；而且**转录路径也会把图一起发出去**（附件先保存、render 见到
    // imageRef 就加图像块），所以那条"兼容路径"在纯文本模型上从来没兼容过。
    const support = await sessionImageSupport()
    const useTranscribe = wantText === true || support.accepts !== true
    result.visionPath = useTranscribe ? 'transcribe' : 'image'
    result.sessionModel = support.model === '' ? '' : ((support.provider === '' ? '' : support.provider + '/') + support.model)

    if (useTranscribe) {
      // 转录路径：不写附件、不返回 image 块。收不了图的模型拿到图像块会让整个请求失败，
      // 那比"这一次没有原图"糟糕得多。
      if (wantText === true && support.accepts === true) {
        result.note = 'transcribe was requested explicitly'
      } else if (support.known === true) {
        result.note = 'the session model (' + result.sessionModel + ') declares no image input, so the capture was transcribed through a vision route instead of being attached'
      } else {
        result.note = 'could not determine whether the session model accepts images, so the capture was transcribed instead of attached'
      }
      try {
        ...
        result.error = 'this session model cannot accept images and the transcription fallback failed: ' + errText(error)
      }
      return result
    }

    // 图像路径：把图存成附件、把引用放进结果，由 render 组装成 image 内容块交给
    // **当前模型**直接看。实测（peek_image 探针）这条路径真的送达。
    // 好处：没有第二层转述、不受另一个模型的推理预算折磨、也不产生第二次调用费用。
    const attachments = ctx.get('attachments')
  ...
  own({
    ...
    output: {
      schema: {
        type: 'object',
        additionalProperties: false,
        required: ['ok', 'source', 'sourceSize', 'region', 'size', 'nonBlack', 'blank', 'bytes', 'summary', 'transcribed', 'vision', 'visionPath', 'sessionModel', 'note', 'error', 'imagePath', 'imageRef'],
        properties: {
          ok: { type: 'boolean' },
          source: { type: 'string' },
      ...
      render(_args, value) {
        ...
        const lines = []
        // 把"源"和"成品"写成两个数，别让人（和模型）把前者当成图幅。
        const scope = value.source + (value.region !== '' ? '（区域 ' + value.region + '）' : '')
        lines.push('截图范围：' + scope + '；源 ' + value.sourceSize + ' → 成品 ' + value.size
          + '，非黑 ' + value.nonBlack + '%，' + value.bytes + ' 字节')
        if (value.transcribed === true) {
          // 把"为什么没给图"说清楚。调用方（人）与 agent 都需要知道：不是抓屏失败，
          // 是会话模型收不了图，所以自动改走了转录。note 里带着具体模型名。
          lines.push('（会话模型' + (value.sessionModel !== '' ? ' ' + value.sessionModel : '') + ' 不接受图像输入，已自动改走转录路径；以下是 ' + value.vision + ' 的文字结果）')
    ...
    async execute(args) {
      ...
      // ok 必须按"实际走的那条路"判定。原来是 hasImage && ...，于是转录路径（它按设计
      // 不返回 imageRef）永远判成失败，render 直接报"看图失败：no capture"——
      // 一条能用的路径被自己的成功判据废掉了。
      const ok = result.visionPath === 'transcribe'
        ? (result.transcribed === true && result.error === '')
        : (hasImage && result.error === '')
      return {
        ok,
        ...
        error: result.error !== '' ? result.error : (ok ? '' : (result.reason || 'no capture')),
