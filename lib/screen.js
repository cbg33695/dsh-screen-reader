...
export function install(ctx) {
  ...
  async function observe(force, focus, windowQuery, region, wantText) {
    state.ticks += 1
    const result = {
    ...
    // 关键改动：默认不再把图送去另一个模型"转录"，而是存成附件、把引用放进结果，
    // 由 render 组装成 image 内容块交给**当前模型**直接看。
    // 实测（peek_image 探针）这条路径真的送达，所以转录桥从"必经之路"降级为
    // "可选的兼容路径"，只有显式要求转录（wantText）时才调模型。
    // 好处：不再丢失细节、不再受那个模型的推理预算折磨、也不再产生第二次调用的费用。
    const attachments = ctx.get('attachments')
    ...
    let ref = null
    try {
      ref = await attachments.saveImage({ data: read.bytes, mediaType: 'image/png', name: 'frame.png' })
    } catch (error) {
  ...
  async function tick(force, focus, windowOverride, regionOverride, wantText) {
    if (state.ticking) {
      state.busySkips += 1
      return {
        ...
        locationKind: '', suggestedRegion: '', locationNote: '', imageRef: null, bytes: 0,
      }
    }
    state.ticking = true
    try {
      ...
      return await observe(force === true, focus, useWindow, useRegion, wantText === true)
    } catch (error) {
  ...
  own({
    name: 'screen_watch',
    description:
      'Start or stop continuous change-driven screen recording. While running, every meaningful change of the watched area is captured into a rolling short-term memory that screen_memory reads. Capturing a frame NEVER calls a model, so recording itself is free — you pay only when screen_memory actually hands frames back to look at. At most one screenshot file exists on disk (overwritten in place). Recording is OFF by default and must be started explicitly.',
    parameters: {
      type: 'object',
    ...
    async execute(args) {
      ...
      let note = ''
      if (on) {
        // 录制不再需要解析视觉路由：抓一帧不调用任何模型。
        // 只有 see_screen({ transcribe: true }) 那条兼容路径才需要它，而那条路径自己会解析。
        note = ' 抓帧不调用模型，所以录制本身不花钱。'
  ...
  own({
    name: 'screen_memory',
    description:
      'Read the rolling short-term screen memory: the recent timeline of what was on this machine, returned as ACTUAL FRAMES rather than as somebody else description. The newest frames come back as image content blocks so the current model can look at them directly, alongside a text timeline of when each frame was taken. Capturing a frame never calls a model, so this memory is free until you read it.',
    parameters: {
      type: 'object',
      additionalProperties: false,
      properties: {
        minutes: { type: 'number', description: 'How many minutes back the timeline should cover (default 10, capped by the rolling window).' },
        frames: { type: 'number', description: 'How many of the NEWEST frames to return as images (default 3, max 6). Each frame costs the same fixed image budget regardless of resolution.' },
      },
    },
    output: {
      schema: {
        type: 'object',
        additionalProperties: false,
        required: ['running', 'watching', 'count', 'ticks', 'capturedFrames', 'skippedFrames', 'busySkips', 'lastError', 'entries', 'frames'],
        properties: {
          running: { type: 'boolean' },
          watching: { type: 'string' },
          count: { type: 'number' },
          ticks: { type: 'number' },
          capturedFrames: { type: 'number' },
      ...
      render(_args, value) {
        ...
        for (const frame of value.frames) {
          if (typeof frame.attachmentId === 'string' && frame.attachmentId !== '') {
            blocks.push({ type: 'image', attachment: frame })
          }
        }
        const lines = []
        lines.push('录制中：' + String(value.running) + '，监视范围：' + value.watching)
        lines.push(
          '累计抓帧 ' + value.ticks + ' 次；存入记忆 ' + value.capturedFrames +
            ' 帧；因未变化或限流跳过 ' + value.skippedFrames +
            ' 帧；因上一次仍在进行而跳过 ' + value.busySkips + ' 帧。',
        )
        lines.push('（存入一帧不调用任何模型，所以持续录制本身不花钱；花钱的是你现在读它）')
        if (value.lastError !== '') lines.push('最近错误：' + value.lastError)
        if (value.frames.length > 0) {
          lines.push('上面 ' + value.frames.length + ' 张图由旧到新，是最近的实际画面。')
        }
        if (value.entries.length === 0) {
          lines.push('记忆时间线为空：可能尚未开始录制，或画面一直没有出现有意义的变化。')
        } else {
          lines.push('--- 时间线（由旧到新，共 ' + value.entries.length + ' 帧）---')
          for (const entry of value.entries) {
            lines.push('[' + entry.at + '] ' + entry.size + (entry.target !== '' ? '  ' + entry.target : ''))
          }
        }
    ...
    async execute(args) {
      ...
      const wantFrames = typeof args?.frames === 'number' && args.frames > 0 ? Math.min(Math.floor(args.frames), 6) : 3
      const floor = Date.now() - minutes * 60000
      const entries = []
      const refs = []
      for (const entry of state.buffer) {
        if (entry.atMs < floor) continue
        entries.push({ at: entry.at, size: entry.size, target: entry.target || '' })
        if (entry.ref !== undefined && entry.ref !== null) refs.push(entry.ref)
      }
      // 取最新的 N 帧，但按由旧到新返回 —— 这样读起来才是时间线。
      const frames = refs.slice(Math.max(0, refs.length - wantFrames))
      return {
        running: state.running,
        watching: state.window
          ? ('窗口 "' + state.window + '"' + (state.region ? ' 区域 ' + state.region : ''))
  ...
  own({
    ...
    async execute(args) {
      ...
      const result = await tick(true, focus, windowQuery, region, args?.transcribe === true)
